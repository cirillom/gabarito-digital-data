...
def _write_question_pdf(path: Path, *, include_all_options: bool = True) -> None:
    ...
    page.insert_text((40, 90), "What is shown in the figure?", fontsize=11)
    ...
    page.insert_text((40, 235), "A) First answer", fontsize=11)
    page.insert_text((40, 265), "B) Second answer", fontsize=11)
    page.insert_text((40, 295), "C) Third answer", fontsize=11)
    if include_all_options:
        page.insert_text((40, 325), "D) Fourth answer", fontsize=11)
    document.save(path)
    document.close()
...
class RichContentTest(unittest.TestCase):
    def test_extracts_local_text_options_images_and_preview_without_network(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            self.assertTrue((directory / rich["assets"][0]["path"]).is_file())
            ...
            self.assertIn("figure-", preview_text)
